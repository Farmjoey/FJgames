...
function resetGame() {
  ...
  // Reset to default values if upgrades aren't purchased or active
  if (!upgrades.widerPipeGap || !activeUpgrades.widerPipeGap) {
    pipeGap = 150;
  } else {
    pipeGap = 180;
  }
  
  if (!upgrades.slowerPipes || !activeUpgrades.slowerPipes) {
    pipeSpeed = 1.5;
  } else {
    pipeSpeed = 1;
  }
  
  if (!upgrades.lowerGravity || !activeUpgrades.lowerGravity) {
    gravity = 0.08;
  } else {
    gravity = 0.05;
  }

  // Keep the current skin active instead of resetting it
  // Only reset to default if no skin is active
  if (!activeUpgrades.blueBirdSkin && 
      !activeUpgrades.makerBirdSkin && 
      !activeUpgrades.oldBirdSkin && 
      !activeUpgrades.skyBirdSkin && 
      !activeUpgrades.gayBirdSkin && 
      !activeUpgrades.greenBirdSkin && 
      !activeUpgrades.redBirdSkin &&
      !activeUpgrades.adminBirdSkin &&
      !activeUpgrades.juleBirdSkin &&
      !activeUpgrades.jonesBirdSkin &&
      !activeUpgrades.monsterBirdSkin &&
      !activeUpgrades.primoBirdSkin &&
      !activeUpgrades.omirpBirdSkin &&
      !activeUpgrades.neergBirdSkin &&
      !activeUpgrades.danskBirdSkin &&
      !activeUpgrades.pixilBirdSkin &&
      !activeUpgrades.ownerBirdSkin &&
      !activeUpgrades.invisibleBirdSkin &&
      !activeUpgrades.countryballBirdSkin &&
      !activeUpgrades.polandBirdSkin) {   
...
function gameLoop() {
  ...
  if (gameOver) {
    ...
    // Update the tournament score when the game ends
...
function selectSkin(type) {
    ...
    activeUpgrades.oldBirdSkin = false;
    activeUpgrades.skyBirdSkin = false;
    activeUpgrades.gayBirdSkin = false;
    activeUpgrades.greenBirdSkin = false;
    activeUpgrades.redBirdSkin = false;
    activeUpgrades.adminBirdSkin = false;
    activeUpgrades.juleBirdSkin = false;
    activeUpgrades.jonesBirdSkin = false;
    activeUpgrades.monsterBirdSkin = false;
    activeUpgrades.primoBirdSkin = false;
    activeUpgrades.omirpBirdSkin = false;
    activeUpgrades.neergBirdSkin = false;
    activeUpgrades.danskBirdSkin = false;
    activeUpgrades.pixilBirdSkin = false;
    activeUpgrades.ownerBirdSkin = false;
    activeUpgrades.invisibleBirdSkin = false;
    activeUpgrades.countryballBirdSkin = false;
    activeUpgrades.polandBirdSkin = false;
    // If unequipping, set to default
    if (isUnequipping) {
        activeUpgrades.defaultSkin = true;
        currentBirdSkin = birdSkins.default;
    } else {
        // Set the active skin based on type
        switch(type) {
            case 'default':
                activeUpgrades.defaultSkin = true;
                currentBirdSkin = birdSkins.default;
